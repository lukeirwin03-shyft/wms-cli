...
def server():
    ...
    Supports auth-protected servers via manual capabilities import.
    """
    pass
...
@click.option('--caps-file', '-c', type=click.Path(exists=True),
              help='Local capabilities XML file (for auth-protected servers)')
...
def server_add(name, url, no_fetch, caps_file, ssh_tunnel, notes):
    ...
    Examples:
        ...
        wms server add test https://test.example.com/wms --no-fetch
...
def server_fetch(name, caps_file):
    ...
    Examples:
        ...
        wms server fetch testweather --caps-file ~/Downloads/caps.xml
    """
    server_config = get_server(name)
    if not server_config:
        click.echo(f"Error: Server '{name}' not found.", err=True)
...
def server_update_caps(name, caps_file):
    ...
    Useful for auth-protected servers.
...
def _resolve_capabilities_source(source: str):
    ...
    1. Registered server name (e.g., "prod", "testweather")
...
def diff_cmd(source_a, source_b, verbose, layers_only, output_json, layer_name, summary, 
    ...
    \b
    Examples:
        wms diff prod test                    # Basic comparison
        wms diff prod test -v                 # Verbose with details
        wms diff prod test --layers-only     # Skip service metadata
        wms diff prod test --layer GFS_Temp  # Single layer deep diff
        wms diff prod test --json            # JSON output
        wms diff prod test --stats           # Show statistics
        wms diff prod test -f added          # Only show added layers
        wms diff prod test -o diff.txt       # Save to file
