...
class ServerConfig:
    ...
    name: str                              # User-friendly name (e.g., "prod", "staging")
...
def add_server(
    ...
    Args:
        name: Unique name for this server (e.g., "prod", "staging")
        url: WMS server URL (the URL to use for actual requests)
        fetch_caps: If True, fetch and cache capabilities immediately
        caps_file: Path to local capabilities XML file (for CAC/auth-protected servers)
